...
function isVideoSubRoute(rawUrl) {
    try {
...
let apiKey = (envGlobal.BUILD_MINIMAX_API_KEY || '').trim() || null;
let redirectUrl = DEFAULT_REDIRECT_PAGE;
...
browser.runtime.onMessage.addListener(async (msg, sender) => {
    ...
    if (message.type === 'VIDEO_TITLE') {
        ...
        const runtimeKey = message.apiKey?.trim();
        if (runtimeKey) {
            apiKey = runtimeKey;
        }
...
async function handleVideoTitle(title, url, tabId) {
    try {
...
async function saveRecord(title, url, decision) {
